...
The `@chimera/tools` package defines the `Executor` interface, the `LocalExecutor` implementation, and the four core tools (`bash`, `read`, `write`, `edit`) consumed by the agent via the Vercel AI SDK `tool()` helper. All filesystem and process access flows through an `Executor` so sandboxing can be added later without changing tool code.
