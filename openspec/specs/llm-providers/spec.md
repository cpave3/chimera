...
The `@chimera/providers` package is a shape-based provider registry for LLMs. It lets users configure any OpenAI-compatible or Anthropic-compatible HTTP endpoint under a local provider alias, resolves `providerId/modelId` references, and handles environment-variable-based API key indirection without leaking secrets.
