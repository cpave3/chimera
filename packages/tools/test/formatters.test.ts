...
describe('buildTools formatters', () => {
  ...
  describe('write', () => {
    ...
    it('appends "(created, N bytes)" when the result reports created=true', () => {
      ...
      expect(
        f(
          { path: join(root, 'new.txt'), content: 'hello' },
          { bytes_written: 5, created: true },
        ).summary,
  ...
  describe('edit', () => {
    it('summarizes by relative path', () => {
      const f = fmts().edit!;
      expect(
        f({ path: join(root, 'src/foo.ts'), old_string: 'a', new_string: 'b' }),
      ).toEqual({ summary: 'src/foo.ts' });
    });
    ...
    it('appends "(N replacements)" when result is known', () => {
      ...
      expect(
        f(
          { path: join(root, 'src/foo.ts'), old_string: 'a', new_string: 'b' },
          { replacements: 3 },
        ).summary,
  ...
  describe('read', () => {
    ...
    it('appends a line range when start_line is given', () => {
      const f = fmts().read!;
      expect(
        f({ path: join(root, 'src/foo.ts'), start_line: 12, end_line: 40 }),
      ).toEqual({ summary: 'src/foo.ts:12-40' });
    });
